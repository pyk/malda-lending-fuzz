    ...
    function batchProcess(BatchProcessMsg calldata data) external {
        if (
            !rolesOperator.isAllowedFor(
                msg.sender, rolesOperator.PROOF_FORWARDER()
            )
        ) {
        ...
        for (uint256 i = 0; i < length;) {
            ...
            if (selector == MINT_SELECTOR) {
                ...
                try ImErc20Host(data.mTokens[i]).repayExternal(
                    encodedJournal, "", singleAmount, data.receivers[i]
                ) {
                    emit BatchProcessSuccess(
                        data.initHashes[i],
                        data.receivers[i],
                ...
                try ImTokenGateway(data.mTokens[i]).outHere(
                    encodedJournal, "", singleAmount, data.receivers[i]
                ) {
                    emit BatchProcessSuccess(
                        data.initHashes[i],
