    ...
    OwnableUpgradeable
{
    ...
    modifier ifNotBlacklisted(address user) {
        require(
            !blacklistOperator.isBlacklisted(user), Operator_UserBlacklisted()
        );
        _;
    }
    ...
    {
        require(
            newCloseFactorMantissa >= CLOSE_FACTOR_MIN_MANTISSA
                && newCloseFactorMantissa <= CLOSE_FACTOR_MAX_MANTISSA,
        ...
        onlyOwner
    {
        ...
        if (
            newCollateralFactorMantissa != 0
                && IOracleOperator(oracleOperator).getUnderlyingPrice(mToken) == 0
        ) {
        ...
        emit NewCollateralFactor(
            mToken, market.collateralFactorMantissa, newCollateralFactorMantissa
        );
        ...
        onlyOwner
    {
        ...
        emit NewLiquidationIncentive(
            market,
            liquidationIncentiveMantissa[market],
    ...
    function supportMarket(address mToken) external onlyOwner {
        require(
            !markets[address(mToken)].isListed, Operator_MarketAlreadyListed()
        );
     ...
     */
    function setOutflowVolumeTimeWindow(uint256 newTimeWindow)
        external
        onlyOwner
    {
    ...
    function checkOutflowVolumeLimit(uint256 amount) external {
        ...
        if (limitPerTimePeriod > 0) {
            ...
            if (
                block.timestamp
                    > lastOutflowResetTimestamp + outflowResetTimeWindow
            ) {
                cumulativeOutflowVolume = 0;
                lastOutflowResetTimestamp = block.timestamp;
            }
            ...
            uint256 amountInUSD =
                _convertMarketAmountToUSDValue(amount, msg.sender);
        ...
        require(
            msg.sender == owner()
                || rolesOperator.isAllowedFor(
                    msg.sender, rolesOperator.GUARDIAN_BORROW_CAP()
                ),
        ...
        external
    {
        require(
            msg.sender == owner()
                || rolesOperator.isAllowedFor(
                    msg.sender, rolesOperator.GUARDIAN_SUPPLY_CAP()
                ),
            Operator_OnlyAdminOrRole()
        );
     ...
     * @inheritdoc IOperator
     */
    function isDeprecated(address mToken)
        external
        view
        override
        returns (bool)
     ...
     * @inheritdoc IOperator
     */
    function isMarketListed(address mToken)
        external
        view
        override
        returns (bool)
        ...
        returns (uint256, uint256)
    {
        return _getHypotheticalAccountLiquidity(
            account, mTokenModify, redeemTokens, borrowAmount
        );
    }
        ...
        returns (uint256)
    {
        ...
        uint256 priceBorrowedMantissa =
            IOracleOperator(oracleOperator).getUnderlyingPrice(mTokenBorrowed);
        uint256 priceCollateralMantissa =
            IOracleOperator(oracleOperator).getUnderlyingPrice(mTokenCollateral);
        require(
        ...
        Exp memory ratio;
        numerator = mul_(
            Exp({mantissa: liquidationIncentiveMantissa[mTokenCollateral]}),
     ...
     * @inheritdoc IOperator
     */
    function enterMarketsWithSender(address _account)
        external
        override
        onlyAllowedUser(_account)
    {
    ...
    function exitMarket(address _mToken) external override {
        ...
        (uint256 tokensHeld, uint256 amountOwed,) =
    ...
    function getUSDValueForAllMarkets() external view returns (uint256) {
        ...
        for (uint256 i; i < allMarkets.length;) {
            ...
            if (_isDeprecated(address(_market))) {
                continue;
            }
            uint256 totalMarketVolume = _market.totalUnderlying();
            sum += _convertMarketAmountToUSDValue(
                totalMarketVolume, address(_market)
            );
            unchecked {
                ++i;
            }
    ...
    function afterMTokenMint(address mToken) external view override {
        ...
        if (supplyCap != 0) {
            ...
            Exp memory exchangeRate =
                Exp({mantissa: ImToken(mToken).exchangeRateStored()});
        ...
        ifNotBlacklisted(borrower)
    {
        ...
        if (!markets[mToken].accountMembership[borrower]) {
            ...
            require(
                markets[mToken].accountMembership[borrower],
        ...
        if (borrowCap != 0) {
            ...
            require(
                nextTotalBorrows < borrowCap, Operator_MarketBorrowCapReached()
            );
        }

        // liquidity check
        (, uint256 shortfall) =
            _getHypotheticalAccountLiquidity(borrower, mToken, 0, borrowAmount);
    ...
    {
        require(
            !_paused[mTokenBorrowed][OperationType.Liquidate], Operator_Paused()
        );
        ...
        uint256 borrowBalance =
            ImToken(mTokenBorrowed).borrowBalanceStored(borrower);

        if (_isDeprecated(mTokenBorrowed)) {
            require(
                borrowBalance >= repayAmount, Operator_RepayAmountNotValid()
            );
        } else {
            (, uint256 shortfall) =
                _getHypotheticalAccountLiquidity(borrower, address(0), 0, 0);
            require(shortfall > 0, Operator_InsufficientLiquidity());
    ...
    {
        require(
            !_paused[mTokenCollateral][OperationType.Seize]
                && !_paused[mTokenBorrowed][OperationType.Seize],
            Operator_Paused()
        );
        require(markets[mTokenBorrowed].isListed, Operator_MarketNotListed());
        require(markets[mTokenCollateral].isListed, Operator_MarketNotListed());
        require(
            ImToken(mTokenCollateral).operator()
                == ImToken(mTokenBorrowed).operator(),
            Operator_Mismatch()
        );
    ...
    // ----------- PRIVATE ------------
    function _convertMarketAmountToUSDValue(
        uint256 amount,
        address mToken
    )
        internal
        ...
        returns (uint256)
    {
        uint256 oraclePriceMantissa =
            IOracleOperator(oracleOperator).getUnderlyingPrice(mToken);
        ...
        view
    {
        ...
        // liquidity check
        (, uint256 shortfall) =
            _getHypotheticalAccountLiquidity(redeemer, mToken, redeemTokens, 0);
        require(shortfall == 0, Operator_InsufficientLiquidity());
    }
        ...
        returns (uint256, uint256)
    {
        ...
        for (uint256 i; i < len;) {
            ...
            vars.collateralFactor =
                Exp({mantissa: markets[_asset].collateralFactorMantissa});
            vars.exchangeRate = Exp({mantissa: vars.exchangeRateMantissa});

            // Get the normalized price of the asset
            vars.oraclePriceMantissa =
                IOracleOperator(oracleOperator).getUnderlyingPrice(_asset);
            require(
     ...
     */
    function _distributeSupplierMalda(
        address mToken,
        address supplier
    )
        private
     ...
     */
    function _distributeBorrowerMalda(
        address mToken,
        address borrower
    )
        private
    ...
    function _isDeprecated(address mToken) private view returns (bool) {
        return markets[mToken].collateralFactorMantissa == 0
            && _paused[mToken][OperationType.Borrow]
